...
def build_unified_feed(articles: pd.DataFrame, events: pd.DataFrame) -> pd.DataFrame:
    ...
    if "heat_score" not in df.columns:
        df["heat_score"] = 0
    df = df.sort_values(["heat_score", "_i", "_q", "time_utc"], ascending=[False, True, True, False]).drop(columns=["_i", "_q"])
    return df
...
st.set_page_config(page_title="Global News Radar V28", layout="wide", initial_sidebar_state="collapsed")
...
st.title("🌍 Global News Radar V28：投資情報雷達穩定版")
...
with tab_feed:
    st.subheader("統合新聞流")
    st.caption("V28：一鍵複製 Markdown 新聞包到剪貼簿，回 ChatGPT 直接貼上即可。")
            ...
            for _, row in feed.head(100).iterrows():
                ...
                st.markdown(
                    ...
                    <div class="article-card">
                        ...
                        <div class="article-meta">{time_utc}<br>{domain}｜{location}｜{quality}<br>熱度分數：{heat_score}<br>翻譯來源：{translation_engine}<br>{summary}</div>
